...
class AboutVC: NSViewController {
    ...
    let supportText = """
    The jamfHelper Constructor application helps build the jamfHelper command and preview the window.
    ...
    Warning: Failure to test jamfHelper Constructor thoroughly on non production devices could result in misconfigurations on production devices.
    ...
    func formattedText() -> NSAttributedString {
        ...
        if issuesRange.location != NSNotFound {
            feedbackString.addAttribute(NSAttributedString.Key.link, value: "https://github.com/BIG-RAT/jhc/issues", range: issuesRange)
        }
    ...
    override func viewDidLoad() {
        ...
        appName_textfield.stringValue = "\(AppInfo.displayname)"
